...
> 經調查，台中市動保處「待認養犬貓照片檢索」列表（149 筆）中，所有動物均已透過農業部 Open Data API 提供，
> 不需要額外爬蟲。三種來源均可從 API 的 `animal_remark`（備註欄）辨識：
>
> | 來源類型 | `所在園區` 欄 | 備註格式 | 地址取得方式 |
> |---------|-------------|---------|------------|
> | 政府收容所 | `南屯園區` / `后里園區` | 無或健康說明 | 固定座標（`KNOWN_LOCATIONS`） |
> | 中途動物醫院 | `中途動物醫院` | `我在 "區域 店名" 等待...電話：XXXX`（**無完整地址**） | 店名 geocoding 或快取 |
> | 益起認養吧 | `益起認養吧` | `我在區域"店名"等待...電話：XXXX 台中市XX區XX路XX號`（**含完整地址**） | 直接從備註解析地址 |
>
> Phase 3 略過（未來若需爬取民眾自行送養佈告欄再實作）。

### Phase 3.5：三種來源資料完整支援（目前實作目標）

**背景**：農業部 API 的台中資料（截至 2026-03-14）共 149 筆，分佈如下：
- 南屯園區：88 筆
- 后里園區：39 筆
- 益起認養吧：15 筆（合作寵物店，有完整地址）
- 中途動物醫院：7 筆（委託動物醫院，地址需從店名推斷）

**待辦**：

- [ ] 擴充 `fetch_animals.py` 的 `_extract_vet_transit_info()`，新增「益起認養吧」與「中途動物醫院」備註格式解析
  - 益起認養吧 regex：`我在.+?等待.+?聯絡電話[：:]([\d\-]+)\s+(.+)`（第 2 組為完整地址）
  - 中途動物醫院 regex：`我在\s*["\s]*([\w\s]+(?:動物醫院|動物診所|獸醫院))["\s]*等待.+?電話[：:]([\d\-]+)`（無地址，用店名 geocoding）
- [ ] 在 `config.py` 新增 `location_type` 常數：`"shelter"` / `"vet_transit"` / `"yiqi"` 以便前端顯示不同標示
- [ ] 更新 `build_data.py`：益起認養吧直接用備註中的地址 geocoding；中途動物醫院用店名查詢
- [ ] 前端 `AnimalCard.vue`：依 `source` 欄位顯示不同來源標籤（政府收容所 / 中途動物醫院 / 益起認養吧）
