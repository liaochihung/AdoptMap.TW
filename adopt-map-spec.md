...
### Phase 3.5：三種來源資料完整支援 ✅

**背景**：農業部 API 的台中資料中，`animal_place` 欄位會出現四種值，資料每日變動：
- 南屯園區：固定座標收容所
- 后里園區：固定座標收容所
- 益起認養吧：合作寵物店，備註含完整地址
- 中途動物醫院：委託動物醫院，地址需從店名推斷

**已完成**：

- [x] 新增 `parse_remark_location()` 取代 `_extract_vet_transit_info()`，正確識別三種來源並解析店名、電話、地址
- [x] 在 `config.py` 新增 `LOCATION_TYPES` 常數：`"shelter"` / `"vet_transit"` / `"yiqi"`
- [x] `build_data.py` 不需改動（已支援 `_location_type` 傳遞）；益起認養吧用備註地址 geocoding，中途動物醫院用店名查詢
- [x] 前端 `AnimalCard.vue`：依 `location.type` 顯示不同來源標籤（公立收容所藍色 / 中途動物醫院綠色 / 益起認養吧紫色）
- [x] 新增 pytest 與 12 個測試（單元 + 整合），全部通過
